...
**Search by text (e.g. “heavy”)**

```http
GET /v1/products?search=heavy
Authorization: Bearer <token>
```
...
```http
GET /v1/products?category=tshirt&brand=Stanley%2FStella&search=medium
Authorization: Bearer <token>
```
...
| `gender`        | string | Gender (`unisex`, `mens`, `ladies`, `kids`,`baby`).   |
